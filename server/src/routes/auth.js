...
authRouter.post("/setup-password", validate(setupPasswordSchema), asyncHandler(async (req, res) => {
  ...
  const accountToken = await transaction(async (client) => {
    // Claim and consume inside one transaction; simultaneous submissions cannot reuse a token.
    const result = await client.query(
      `SELECT t.id, t.user_id, u.email FROM user_account_tokens t JOIN users u ON u.id=t.user_id
       WHERE t.token_hash=$1 AND t.used_at IS NULL AND t.expires_at>now() AND u.active=true AND u.deleted_at IS NULL
       FOR UPDATE OF t, u`, [tokenHash]);
    const token = result.rows[0];
    if (!token) throw new AppError("This setup link is invalid or expired.", 400, "INVALID_SETUP_TOKEN");
    await client.query("UPDATE users SET password_hash=$1, invitation_status='ACTIVE', updated_at=now() WHERE id=$2", [passwordHash, token.user_id]);
    await client.query("UPDATE user_account_tokens SET used_at=now() WHERE user_id=$1 AND used_at IS NULL", [token.user_id]);
    await client.query("UPDATE user_sessions SET revoked_at=now() WHERE user_id=$1 AND revoked_at IS NULL", [token.user_id]);
    await client.query("INSERT INTO audit_logs(user_id, action, entity, entity_id) VALUES($1,'account_password_set','user',$1)", [token.user_id]);
    return token;
  });
