...
export function attributionFrom(payload={}){
...
export async function captureAttribution(leadId,payload){
 const touch=attributionFrom(payload);if(!Object.keys(touch).length)return;
...
export async function enqueueIntegrationEvent({provider,eventName,entityType,entityId,payload={},idempotencyKey}){
 ...
 ON CONFLICT(idempotency_key) DO UPDATE SET idempotency_key=EXCLUDED.idempotency_key RETURNING *`,[provider,eventName,entityType,entityId,JSON.stringify(safe),idempotencyKey]);return row.rows[0];
}
const lifecycleEvents={lead_created:'generate_lead',proposal_sent:'proposal_sent',proposal_accepted:'proposal_accepted',lead_converted:'booking_created',booking_auto_confirmed:'booking_confirmed',invoice_created:'invoice_issued',payment_succeeded:'payment_completed',event_completed:'event_completed'};
export async function enqueueLifecycle({action,entityType,entityId}){
 ...
 else if(entityType==='invoice'||entityType==='payment')lead=(await query(`SELECT c.first_touch FROM ${entityType==='invoice'?'invoices':'payments'} r JOIN clients c ON c.id=r.client_id WHERE r.id=$1`,[entityId])).rows[0];
 for(const provider of ['GA4','META','TIKTOK'])await enqueueIntegrationEvent({provider,eventName,entityType,entityId,payload:{attribution:lead?.first_touch},idempotencyKey:`${provider}:${eventName}:${entityId}`});
}
...
export async function smsEligibility({entityType,entityId,rule,reason}){
 ...
 else if(entityType==='invoice')recipient=(await query('SELECT c.*,i.due_date,i.amount_outstanding FROM invoices i JOIN clients c ON c.id=i.client_id WHERE i.id=$1 AND i.deleted_at IS NULL',[entityId])).rows[0];
 ...
 const due=(await query("SELECT due_date <= current_date-$2::int AND amount_outstanding>0 AND status NOT IN ('VOID','PAID','REFUNDED') AS eligible FROM invoices WHERE id=$1",[entityId,Number(settings.overdue_days)||7])).rows[0];if(!due?.eligible)return {allowed:false,reason:'Balance escalation criteria not met.'};
 }
...
export async function processIntegrationJobs({limit=25,dispatch=developmentDispatch}={}){
 if(dispatch!==developmentDispatch&&env.nodeEnv!=='test')throw new AppError('Test adapter unavailable.',403,'TEST_ONLY');
 await query("UPDATE integration_jobs SET status='QUEUED',available_at=now() WHERE status='PROCESSING' AND started_at<now()-interval '5 minutes'");
 const results=[];
 ...
 await dispatch(job);await client.query("UPDATE integration_jobs SET status='SUCCEEDED',completed_at=now(),last_error=NULL WHERE id=$1",[job.id]);});results.push({id:job.id,status:'PROCESSED'});
 }catch(error){await query("UPDATE integration_jobs SET status=$2,last_error=$3,available_at=now()+interval '1 minute' WHERE id=$1",[job.id,job.attempts>=3?'FAILED':'QUEUED',error.code||'INTEGRATION_DISPATCH_FAILED']);results.push({id:job.id,status:'RETRY_OR_FAILED'});}
 }
 return results;
}
...
export async function queueDueReminders(){
 ...
 const results=[];for(const event of events)results.push(await queueEventReminder(event.id));return results;
}
