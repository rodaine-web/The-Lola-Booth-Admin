...
try{
 ...
 await check('Analytics precise currency',async()=>{await page.goto('https://admin.test/insights/analytics');await page.getByRole('heading',{name:'Analytics',exact:true}).waitFor();await page.getByText('average booking value',{exact:false}).waitFor({timeout:2000});assert.ok((await page.locator('main').innerText()).includes('$999.20'));});
