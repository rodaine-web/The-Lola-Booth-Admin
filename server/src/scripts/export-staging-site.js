...
for(const file of files){if(file.endsWith('.js'))await fs.writeFile(path.join(target,file),(await fs.readFile(path.join(root,file),'utf8')).split('\n').map(line=>line.trimEnd()).join('\n').trimEnd()+'\n');else await fs.copyFile(path.join(root,file),path.join(target,file));}
