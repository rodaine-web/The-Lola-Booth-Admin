...
export default function MediaSelect({value,onChange}){
 ...
 return <span className="relationship-select"><input aria-label="Search media library" placeholder="Search media by name" value={search} onChange={e=>setSearch(e.target.value)}/><select value={value||''} onChange={e=>onChange(e.target.value||null)}><option value="">Select media…</option>{value&&!items.some(i=>i.id===value)&&<option value={value}>Current selected media</option>}{items.map(i=><option key={i.id} value={i.id}>{i.original_filename||i.filename} · {i.visibility} / {i.permission_state}</option>)}</select>{error&&<small role="alert">{error}</small>}</span>;
}
